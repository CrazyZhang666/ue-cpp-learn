...
```
                          ...
                          │                                              │
                          │  学会GAS后，你会更容易理解大型UE项目的架构      │
                          │                                              │
                          │  本章先建立地图，不要求一次记住所有API          │
                          └─────────────────────────────────────────────┘
```

GAS（Gameplay Ability System）是Epic为UE4/UE5开发的一套**技能系统框架**。它最初是为《堡垒之夜》和《Paragon》开发的，经过多年实战检验，现在是UE5官方推荐的游戏逻辑核心系统。

用一句话概括：**GAS是一套用数据驱动方式搭建角色属性、技能、Buff/Debuff、状态管理的框架，内置网络同步支持。**

---

## 新手先读：GAS先记五个积木

GAS术语很多，第一遍只需要把下面五个积木对应起来：

| 积木 | 新手先理解成 |
| ---- | ------------ |
| `AbilitySystemComponent` | GAS总管，挂在角色身上，负责技能和效果 |
| `GameplayAbility` | 一个技能，例如火球、翻滚、治疗 |
| `GameplayEffect` | 一个效果，例如扣血、回血、加速、冷却 |
| `AttributeSet` | 一组属性，例如生命、法力、攻击力 |
| `GameplayTag` | 标签语言，例如“正在眩晕”“技能冷却中” |

后面所有章节都在围绕这五个积木展开。读不懂时先问自己：这段代码是在创建哪个积木？它和其他积木怎么连接？

> **阅读建议**：第一遍读GAS不要强求理解网络同步和预测。先理解单机流程：技能激活 → 应用效果 → 修改属性 → UI显示变化。
...
**核心思想**：GAS把"技能的共性需求"（消耗检测、冷却管理、状态判断、网络同步）都帮你处理好了。你只需要**关注技能本身的独特逻辑**。
...
当玩家需要判断"能否释放技能"时，只需要检查 `!HasAny(State.CC, State.Dead)`。以后新增的任何控制效果（只要它的标签以`State.CC`开头），都会被自动拦截，一行代码都不用改。

### 问题5：网络同步

多人在线游戏中，技能系统的网络同步是容易出Bug的地方。
...
```
❌ 误区1："GAS只能做战斗技能"
   ✅ 事实：GAS可以做跳跃、冲刺、交互、对话、背包系统等各种事情
   只要是需要"条件判断+执行逻辑+状态管理"的系统，都可以用GAS。

❌ 误区2："GAS太重了，不适合小项目"
   ✅ 事实：GAS的开销主要在开发时的学习成本，运行时很高效。
   即使小项目，如果核心玩法涉及技能和属性，GAS反而能加速开发。

❌ 误区3："GAS必须配合蓝图使用"
   ✅ 事实：GAS的C++ API很完善，可以用纯C++。
   蓝图是可选的便捷补充，不是必须。
...
> **下一节**：[13.2 AbilitySystemComponent](./02-AbilitySystemComponent.md) — GAS的大脑核心
